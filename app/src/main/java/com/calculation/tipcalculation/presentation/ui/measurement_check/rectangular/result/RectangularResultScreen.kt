package com.calculation.tipcalculation.presentation.ui.measurement_check.rectangular.result
...
import com.calculation.tipcalculation.presentation.view_model.measurement_check.rectangular.result.RectangularResultViewModel

@Composable
fun RectangularResultScreen(
    navController: NavController,
    viewModel: RectangularResultViewModel = hiltViewModel()
) {
                ...
                item {
                    result?.let { it ->
                        BoxCard(modifier = Modifier.fillMaxWidth()) {
                            ...
                            ResultText("Ширина A", "%.2f мм".format(it.a))
                            ResultText("Высота B", "%.2f мм".format(it.b))
                            ResultText("Длина L", "%.2f мм".format(it.l)) 
                            ...
                            ResultText(
                                "Количество точек",
                                "${it.rule.totalPoints} (${it.rule.na}×${it.rule.nb})"
                            )
